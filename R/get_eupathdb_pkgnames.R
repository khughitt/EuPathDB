...
=======
#' @param eupathdb_version Choose a specific version of the eupathdb, only really useful
#'   when downloading files.
...
#' @export
<<<<<<< HEAD
<<<<<<< HEAD:R/get_eupath_pkgnames.R
get_eupath_pkgnames <- function(entry, eu_version = NULL, column = "TaxonUnmodified") {
=======
get_eupathdb_pkgnames <- function(entry, eu_version=NULL, column="TaxonUnmodified") {
>>>>>>> fd9c661 (Doing a bit of re-organizing):R/get_eupathdb_pkgnames.R
=======
get_eupathdb_pkgnames <- function(entry, eupathdb_version = NULL, column = "TaxonUnmodified") {
>>>>>>> cc20d16 (Continuing clean-up / re-organization)
  ...
  if (!is.null(eupathdb_version)) {
    ...
    version_string <- glue::glue(".{eupathdb_version}")
  }
  ...
  pkg_list <- list(
    ...
    txdb        = glue::glue("TxDb.{taxa[['genus']]}.{taxa[['species_strain']]}.{entry[['DataProvider']]}{version_string}"),
