...
#' @author atb
#' @export
<<<<<<< HEAD:R/extract_eupath_orthologs.R
...
extract_eupathdb_orthologs <- function(db, master="GID", query_species=NULL,
  ...
  load_pkg <- function(name, ...) {
    ...
    if (class(first_try) == "try-error") {
      ...
      entry <- get_eupathdb_entry(name)
      pkg_names <- get_eupathdb_pkgnames(entry)
      first_pkg <- pkg_names[["orgdb"]]
      tt <- try(do.call("library", as.list(first_pkg)), silent=TRUE)
      if (class(tt) == "try-error") {
        message("Did not find the package: ",
                first_pkg,
                ". Will not be able to do reciprocal hits.")
        message("Perhaps try invoking make_eupathdb_organismdbi().")
        pkg <- NULL
      } else {
        message("Loaded: ", first_pkg)
        pkg <- get(first_pkg)
...
>>>>>>> fd9c661 (Doing a bit of re-organizing):R/extract_eupathdb_orthologs.R
