...
if (file.exists(meta_valid_files[["orgdb"]])) {
  message("Using extant valid and invalid metadata files.")
  ...
  message("Downloading new metadata files.")
...
for (it in seq_len(valid_end)) {
  ...
  if (isTRUE(do_orgdb)) {
    ...
    ## I have an idea for these tests: the metadata entry should include some
    ## information regarding the likely number of annotated genes etc.
    ## So we should be able to check them against each other here.
